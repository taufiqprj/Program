/*
PROJECT : BARONAS MECHANUM ROBOT TRANSPORTER
AUTHOR  : WILDAN TAUFIQIE D
*/
/////////////////PROOGRAM KHUSUS ROBOT BARU (STICK HITAM) LAMPU RGB
//ATUR SPEED DISINI
int speed = 180;  //TOMBOL
int speedAnalogKanan = 165;
int speedAnalogKiri = 160;
int speedTurbo = 230;
int speed_basic = 0;

//ATUR NILAI SUDUT SERVO DISINI
int nilaiKubusTutup = 60;
int nilaiKubusBuka = 5;

int nilaiBola1Buka = 65;  // TOMBOL O //KETIKA TUTUP
int nilaiBola1Tutup = 0;

int nilaiBola2Buka = 120;  // TOMBOL X
...
*/
int nilaiKubusAngkat1 = 62;
int nilaiKubusAngkat2 = 0;
int nilaiKubusTurun = 180;
...
////////////////////////////////>>>>>>>> JANGAN DIUBAH TANPA SEIZIN AUTHOR <<<<<<<</////////////////////
//JOYSTICK
...
void loop() {
  ...
  if (starting == 1) {
    ...
    if (lx > 2) {  //geser kanan pelan
      set_motor(speedAnalogKiri + 10, -speedAnalogKiri - 10, -speedAnalogKiri - 10, speedAnalogKiri + 10);
      delay(100);
    }
    ...
    if (keyCross) {  //atur gripper bola 2
      if (gripBola2) {
        ...
        moveServo(pinGripBola2, nilaiBola2);
    ...
    if (keySquare && (!keyR2 || keyL1)) {
      ...
      // }
      nilaiUpKubus = nilaiKubusTurun;  //30 || 10
      moveServoUp(pinUpKubus, nilaiUpKubus);
      delay(250);
    }
