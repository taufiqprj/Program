/////////////////PROOGRAM KHUSUS ROBOT LAMA (STICK MERAH)
//ATUR SPEED DISINI
int speed = 200;
...
int nilaiKubusTutup = 65;
int nilaiKubusBuka = 5;

int nilaiBola1Buka = 65;  // TOMBOL O
...
void loop() {
  ...
  if (starting == 1) {
    ...
    if (keyUp) {  //maju
      ...
      set_motor(-speed - 10, speed + 10, speed, -speed);
      delay(100);
    } else {
      set_motor(0, 0, 0, 0);
    }
    ...
    if (keySquare && !(keyR2 || keyL1)) {
      ...
      delay(250);
    }else if (keySquare && (!keyR2 || keyL1)) {
      // while (nilaiUpKubus < nilaiKubusTurun) {
      //   nilaiUpKubus = nilaiUpKubus + 30; //30 || 10
      //   moveServoUp(pinUpKubus, nilaiUpKubus);
      // }
